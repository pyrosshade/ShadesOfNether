...
public class ScreamerSurfaceBuilder extends SurfaceBuilder<SurfaceBuilderConfig>{
	private static final BlockState CAVE_AIR = Blocks.CAVE_AIR.getDefaultState();
	private static final BlockState GRAVEL = Blocks.GRAVEL.getDefaultState();
	private static final BlockState SOUL_SAND = Blocks.SOUL_SAND.getDefaultState();
	private static final BlockState SCREAMERRACK = ForgeRegistries.BLOCKS.getValue(new ResourceLocation("shadesofnether:screamerrack")).getDefaultState();
	protected long field_205552_a;
	protected OctavesNoiseGenerator field_205553_b;
	
	public ScreamerSurfaceBuilder(Function<Dynamic<?>, ? extends SurfaceBuilderConfig> p_i51305_1_) {
		super(p_i51305_1_);
	}
	...
	public void buildSurface(Random random, IChunk chunkIn, Biome biomeIn, int x, int z, int startHeight, double noise,
		...
		int i = seaLevel + 1;
	      ...
	      int i1 = -1;
	      BlockState blockstate = SCREAMERRACK;
	      BlockState blockstate1 = SCREAMERRACK;
	         ...
	         if (blockstate2.getBlock() != null && !blockstate2.isAir()) {
	            if (blockstate2.getBlock() == defaultBlock.getBlock()) {
	               if (i1 == -1) {
	                  if (l <= 0) {
	                     blockstate = CAVE_AIR;
	                     blockstate1 = SCREAMERRACK;
	                  } else if (j1 >= i - 4 && j1 <= i + 1) {
	                     blockstate = SCREAMERRACK;
	                     blockstate1 = SCREAMERRACK;
	                     if (flag1) {
	                        blockstate = GRAVEL;
	                        blockstate1 = SCREAMERRACK;
